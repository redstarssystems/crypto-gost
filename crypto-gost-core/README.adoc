...
<3> Начиная с версии 0.5.0 каждый поток выводит два независимых ключа
(`cmacKey` для CMAC и `ctrKey` для CTR) через KDF_TREE_GOSTR3411_2012_256
из мастер-ключа и случайного streamNonce. Один мастер-ключ можно безопасно
использовать для произвольного числа потоков — производные ключи разных
потоков вычислительно независимы, компрометация одного потока не затрагивает
остальные.
