...
Через JCA (`org.rssys.gost.jca`)
    ...
    try {
        ...
        if (key instanceof Destroyable d) {
            d.destroy(); // обнуляет ключевой материал в памяти
        }
    }
...
Генерация ключевой пары, подпись и верификация
    ...
    try {
        ...
        if (pair.getPrivate() instanceof Destroyable d) {
            d.destroy();
        }
    }
...
Импорт закрытого ключа из PKCS#8 DER и подпись
    ...
    try {
        ...
        if (priv instanceof Destroyable d) {
            d.destroy();
        }
    }
...
### VKO через JCA
    ...
    // Генерация ключевых пар через JCA — для VKO нужны именно JCA-ключи
    KeyPairGenerator kpg = KeyPairGenerator.getInstance("ECGOST3410-2012", "RssysGostProvider");
    kpg.initialize(new ECGenParameterSpec("cryptopro-A"));
    KeyPair aliceJca = kpg.generateKeyPair();
    KeyPair bobJca   = kpg.generateKeyPair();
    ...
    ka.init(aliceJca.getPrivate(), new GostUkmParameterSpec(ukm)); // 
    ka.doPhase(bobJca.getPublic(), true);
...
api Raw ECDH
    ...
    // myPriv — PrivateKeyParameters, peerPub — PublicKeyParameters
...
Через JCA (`ECDHGOST2012`) — аналогично, без UKM:
    ...
    // myPriv — java.security.PrivateKey (из JCA KeyPairGenerator),
    // peerPub — java.security.PublicKey (из JCA KeyPairGenerator)
...
Готовая реализация — в модуле `crypto-gost-pkix`:
`CmsEnvelopedDataBuilder` / `CmsEnvelopedDataDecryptor`. См. [CMS
EnvelopedData](../crypto-gost-pkix/README.adoc).
