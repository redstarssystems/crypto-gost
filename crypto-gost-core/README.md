...
- Начиная с версии 0.5.0 каждый поток выводит два независимых ключа
  (`cmacKey` для CMAC и `ctrKey` для CTR) через
  KDF\_TREE\_GOSTR3411\_2012\_256 из мастер-ключа и случайного
  streamNonce. Один мастер-ключ можно безопасно использовать для
  произвольного числа потоков — производные ключи разных потоков
  вычислительно независимы, компрометация одного потока не затрагивает
  остальные.
...
<tbody>
<tr>
<td style="text-align: left;"><p><code>MGM</code></p></td>
<td style="text-align: left;"><p>Сообщения, файлы с AEAD</p></td>
<td style="text-align: left;"><p>Да (встроенная)</p></td>
