...
Каждый вызов `sealing` должен использовать уникальный ключ или
уникальный контекст — переиспользование одного ключа для нескольких
потоков ослабляет защиту от подмены чанков между сессиями.
