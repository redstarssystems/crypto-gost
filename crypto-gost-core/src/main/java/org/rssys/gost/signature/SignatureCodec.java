 ...
 *
 * <p>Формат: конкатенация r ∥ s в big-endian, каждая компонента занимает
...
public final class SignatureCodec {
    ...
    /**
     * Кодирует компоненты подписи r и s в формат r ∥ s big-endian.
     *
     ...
     * @return байты подписи r ∥ s, длина {@code 2 * rolen}
     */
    public static byte[] encode(BigInteger r, BigInteger s, ECParameters params) {
        ...
        System.arraycopy(rBytes, Math.max(0, rBytes.length - rLen), out, rolen - rLen,     rLen);
        System.arraycopy(sBytes, Math.max(0, sBytes.length - sLen), out, 2 * rolen - sLen, sLen);
        return out;
    }

    /**
     * Декодирует подпись r ∥ s big-endian в компоненты {@link BigInteger}.
     *
     * @param signature байты подписи r ∥ s
    ...
    public static BigInteger[] decode(byte[] signature, ECParameters params) {
        ...
        BigInteger r = new BigInteger(1, Arrays.copyOfRange(signature, 0,     rolen));
        BigInteger s = new BigInteger(1, Arrays.copyOfRange(signature, rolen, 2 * rolen));
