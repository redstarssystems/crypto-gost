...
class ModesTest {
    ...
    static byte[] iv16()  { return hex(IV16_HEX); }
    static byte[] iv32()  { return hex(IV32_HEX); }
    static byte[] iv8()   { return hex(IV8_HEX); }
    ...
    void testCBCEncrypt() {
        ...
        for (int i = 0; i < 4; i++)
            cbc.processBlock(plain, i * 16, result, i * 16);

        assertEquals(CBC_CIPHER_HEX, hex(result),
    ...
    void testCBCDecrypt() {
        ...
        byte[] result = new byte[64];
        for (int i = 0; i < 4; i++)
            cbc.processBlock(cipher, i * 16, result, i * 16);

        assertEquals(PLAIN_HEX, hex(result),
            "CBC-расшифрование должно восстановить открытый текст");
    }

    @Test
    @DisplayName("CBC: encrypt(decrypt(x)) == x для произвольных данных")
    void testCBCRoundtrip() {
        byte[] original = "Тестовая строка для CBC режима!!".getBytes(java.nio.charset.StandardCharsets.UTF_8);
    ...
    void testCBCChaining() {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            assertFalse(Arrays.equals(b1, b2), "Блок " + i + " должен отличаться из-за CBC цепочки");
        }
    }
    ...
    void testCTREncrypt() {
        ...
        assertEquals(CTR_CIPHER_HEX, hex(result),
    ...
    void testCTRShortData() {
        ...
        assertArrayEquals(original, recovered, "CTR должен корректно работать с данными короче блока");
    }
    ...
    void testOFBEncrypt() {
        ...
        assertEquals(OFB_CIPHER_HEX, hex(result),
    ...
    void testOFBDecrypt() {
        ...
        assertEquals(PLAIN_HEX, hex(result),
            "OFB расшифрование должно восстановить открытый текст");
    }
    ...
    void testOFBFirst2BlocksEqualCFB() {
        ...
        assertFalse(
            Arrays.equals(Arrays.copyOfRange(ofbOut, 32, 64),
                          Arrays.copyOfRange(cfbOut, 32, 64)),
    ...
    void testOFBShortData() {
        ...
        assertArrayEquals(original, recovered, "OFB должен корректно работать с данными короче блока");
    }
