...
class GostCipherTest {
    ...
    private static final byte[] DATA_48 =
            "0123456789ABCDEF0123456789ABCDEF0123456789ABCDEF".getBytes();
    private static final byte[] DATA_ODD =
            "нечётное число байт".getBytes(java.nio.charset.StandardCharsets.UTF_8);
    ...
    // -----------------------------------------------------------------------
    // Параметризованные roundtrip-тесты всех режимов
    // -----------------------------------------------------------------------

    @ParameterizedTest
    @DisplayName("roundtrip encrypt/decrypt через JCA")
    @CsvSource({
        "Kuznyechik/CTR/NoPadding, 8",
        "Kuznyechik/CBC/PKCS5Padding, 16",
        "Kuznyechik/OFB/NoPadding, 16",
        "Kuznyechik/CFB/NoPadding, 16"
    })
    void testModeRoundtrip(String algorithm, int ivLen) throws Exception {
        ...
        javax.crypto.Cipher enc = javax.crypto.Cipher.getInstance(algorithm, PROVIDER);
        enc.init(javax.crypto.Cipher.ENCRYPT_MODE, key, ivSpec);
        byte[] ciphertext = enc.doFinal(DATA_48);

        javax.crypto.Cipher dec = javax.crypto.Cipher.getInstance(algorithm, PROVIDER);
        dec.init(javax.crypto.Cipher.DECRYPT_MODE, key, ivSpec);
        byte[] decrypted = dec.doFinal(ciphertext);

        assertArrayEquals(DATA_48, decrypted, algorithm + " roundtrip должен восстановить данные");
    }
    ...
    void testCtrMatchesReferenceApi() throws Exception {
        ...
        // JCA
        javax.crypto.Cipher jca =
                javax.crypto.Cipher.getInstance("Kuznyechik/CTR/NoPadding", PROVIDER);
    ...
    // -----------------------------------------------------------------------
    // CBC (roundtrip параметризован выше, здесь — специфичные тесты)
    // -----------------------------------------------------------------------
    ...
    void testCbcPkcs5OddLength() throws Exception {
        ...
        javax.crypto.Cipher enc =
                javax.crypto.Cipher.getInstance("Kuznyechik/CBC/PKCS5Padding", PROVIDER);
        enc.init(javax.crypto.Cipher.ENCRYPT_MODE, key, ivSpec);
    ...
    void testCbcMatchesReferenceApi() throws Exception {
        ...
        // Эталонный API
        byte[] refResult =
                Cipher.encrypt(DATA_48, keyParam, iv, Cipher.Mode.CBC, Cipher.Padding.PKCS7);
    ...
    void testCbcNoPadding() throws Exception {
        ...
        javax.crypto.Cipher enc =
                javax.crypto.Cipher.getInstance("Kuznyechik/CBC/NoPadding", PROVIDER);
        enc.init(javax.crypto.Cipher.ENCRYPT_MODE, key, ivSpec);
    ...
    // -----------------------------------------------------------------------
    // OFB (roundtrip параметризован выше)
    // -----------------------------------------------------------------------

    @Test
    @DisplayName("OFB/NoPadding: roundtrip с данными произвольной длины")
    void testOfbOddLength() throws Exception {
        SymmetricKey keyParam = KeyGenerator.generateSymmetricKey();
        GostSecretKey key = new GostSecretKey("Kuznyechik", keyParam);
        byte[] iv = new byte[16];
        CryptoRandom.INSTANCE.nextBytes(iv);
        ...
        javax.crypto.Cipher enc =
                javax.crypto.Cipher.getInstance("Kuznyechik/OFB/NoPadding", PROVIDER);
        enc.init(javax.crypto.Cipher.ENCRYPT_MODE, key, ivSpec);
        byte[] ct = enc.doFinal(DATA_ODD);
    ...
    // -----------------------------------------------------------------------
    // CFB (roundtrip параметризован выше)
    // -----------------------------------------------------------------------

    @Test
    @DisplayName("CFB/NoPadding: roundtrip с произвольными данными")
    void testCfbOddLength() throws Exception {
        SymmetricKey keyParam = KeyGenerator.generateSymmetricKey();
        GostSecretKey key = new GostSecretKey("Kuznyechik", keyParam);
        byte[] iv = new byte[16];
        CryptoRandom.INSTANCE.nextBytes(iv);
        ...
        javax.crypto.Cipher enc =
                javax.crypto.Cipher.getInstance("Kuznyechik/CFB/NoPadding", PROVIDER);
        enc.init(javax.crypto.Cipher.ENCRYPT_MODE, key, ivSpec);
        byte[] ct = enc.doFinal(DATA_ODD);
    ...
    void testOmacDecryptViaJca() throws Exception {
        ...
        javax.crypto.Cipher enc =
                javax.crypto.Cipher.getInstance("Kuznyechik/CTR-ACPKM-OMAC/NoPadding", PROVIDER);
        enc.init(javax.crypto.Cipher.ENCRYPT_MODE, key, ivSpec);
