...
----
NOTE: `loadPrivateKey` авто-определяет PEM или DER и поддерживает расшифровку
...
`GostX509KeyManager.asClientCertificateSelector()` автоматически выбирает
подходящий сертификат из KeyStore:
...
NOTE: Записи перебираются в порядке добавления через `addKeyEntry()`.
