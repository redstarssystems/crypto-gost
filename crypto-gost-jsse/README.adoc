...
в уровне записей и JSSE-обёртки. Каждый `@FuzzTest` содержит seed-файлы
...
WARNING: В режиме `JAZZER_FUZZ=1` фаззер генерирует десятки тысяч
вариантов входных данных. На слабом оборудовании это может занять
длительное время. Прервать можно через Ctrl+C — seed-файлы сохраняются
