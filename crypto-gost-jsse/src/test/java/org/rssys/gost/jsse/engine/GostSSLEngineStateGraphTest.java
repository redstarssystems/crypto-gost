...
class GostSSLEngineStateGraphTest {
    ...
    void testDataStateWrap() throws Exception {
        ...
        assertEquals(SSLEngineResult.Status.OK, r.getStatus(),
                "wrap в DATA должен вернуть OK");
        assertEquals(SSLEngineResult.HandshakeStatus.NOT_HANDSHAKING,
                r.getHandshakeStatus(),
                "handshakeStatus в DATA — NOT_HANDSHAKING");
        assertEquals(3, r.bytesConsumed(),
                "bytesConsumed = длина входных данных");
        assertTrue(r.bytesProduced() > 0,
                "bytesProduced > 0 — произведена TLS-запись");
    }
    ...
    void testDataStateUnwrap() throws Exception {
        ...
        assertEquals(SSLEngineResult.Status.OK, r.getStatus(),
                "unwrap в DATA должен вернуть OK");
        assertEquals(SSLEngineResult.HandshakeStatus.NOT_HANDSHAKING,
                r.getHandshakeStatus());
        assertTrue(r.bytesConsumed() > 0,
                "bytesConsumed > 0 — запись потреблена");
        assertTrue(r.bytesProduced() > 0,
                "bytesProduced > 0 — данные расшифрованы");

        appBuf.flip();
        assertEquals(0x42, appBuf.get(0),
                "расшифрованный байт должен совпадать");
    }
    ...
    void testDataStateCloseOutbound() throws Exception {
        ...
        // WHY: closeOutbound() только инициирует отправку close_notify.
        ...
        assertTrue(r.bytesProduced() > 0,
                "close_notify запись должна быть произведена");
        assertEquals(SSLEngineResult.HandshakeStatus.NOT_HANDSHAKING,
                r.getHandshakeStatus());
        assertTrue(server.isOutboundDone(),
                "isOutboundDone после closeOutbound");
    }
    ...
    void testDataStateCloseInbound() throws Exception {
        ...
        assertTrue(server.isInboundDone(),
                "isInboundDone после closeInbound");
        assertFalse(server.isOutboundDone(),
                "isOutboundDone должен оставаться false");
    }

    @Test
    @DisplayName("DATA: closeOutbound + closeInbound → wrap/unwrap бросают SSLException (engine closed)")
    ...
    private static void doHandshake(GostSSLEngine client, GostSSLEngine server,
        ...
        Thread ct = new Thread(() -> {
            try { hsLoop(client, pair.getClientTransport()); }
            catch (Exception e) { ce.set(e); }
        });
        Thread st = new Thread(() -> {
            try { hsLoop(server, pair.getServerTransport()); }
            catch (Exception e) { se.set(e); }
        });
        ct.start(); st.start();
    ...
    private static void hsLoop(GostSSLEngine engine,
        ...
        for (int i = 0; i < 80; i++) {
            ...
            switch (hs) {
                ...
                case NEED_TASK: { Runnable t = engine.getDelegatedTask(); if (t != null) t.run(); break; }
            }
        }
        throw new RuntimeException("HS timeout " + engine.getHandshakeStatus());
    }
}
