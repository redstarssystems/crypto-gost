...
class GostOcspPolicyTest {
    ...
    void testStaplingOrFetchNoFetcher() throws Exception {
        ...
        assertThrows(CertificateException.class,
                ...
                "STAPLING_OR_FETCH without fetcher and without OCSP should fail");
    }
    ...
    void testIfPresentNoOcsp() throws Exception {
        ...
        GostX509TrustManager tm = new GostX509TrustManager(
                root.cert.getPublicKey(), OcspPolicy.IF_PRESENT, null);
    ...
    void testStaplingOrFetchStapled() throws Exception {
        ...
        byte[] ocspResp = TlsTestHelper.buildOcspResponse(
    ...
    void testStaplingOrFetchViaAia() throws Exception {
        ...
        byte[] aiaExt = TlsTestHelper.buildAiaOcspExtensionBytes(ocspUri);
        ...
        mockServer.createContext("/ocsp", ex -> {
        ...
        assertNotNull(recordedBody.get(), "Mock server should have received a request");
        assertTrue(recordedBody.get().length > 0,
    ...
    void testOcspCacheHit() throws Exception {
        ...
        List<TlsCertificate> chain1 = List.of(new TlsCertificate(leafDer), root.cert);
        tm.validateChainWithOcsp(chain1, null, false);
        assertEquals(1, counter.get(), "First call should make HTTP request");

        // Второй вызов — новый объект сертификата (симулирует новый handshake)
        // hasOcspResponse=false → cache lookup → hit → setOcspResponse
        List<TlsCertificate> chain2 = List.of(new TlsCertificate(leafDer), root.cert);
        tm.validateChainWithOcsp(chain2, null, false);
        assertEquals(1, counter.get(), "Second call should hit cache, no HTTP");
        assertTrue(chain2.get(0).hasOcspResponse(), "Cache should set OCSP response on second cert");
    }
    ...
    void testOcspCacheExpiry() throws Exception {
        ...
        List<TlsCertificate> chain1 = List.of(new TlsCertificate(leafDer), root.cert);
        tm.validateChainWithOcsp(chain1, null, false);
        assertEquals(1, counter.get(), "First call should fetch");
        ...
        List<TlsCertificate> chain2 = List.of(new TlsCertificate(leafDer), root.cert);
        tm.validateChainWithOcsp(chain2, null, false);
        assertEquals(2, counter.get(), "After TTL expiry, should refetch");
    }
    ...
    void testIntermediateChainFull() throws Exception {
        ...
        TlsTestHelper.CertBundle intermediate = TlsTestHelper.createCertSignedBy(
                ...
                new byte[]{(byte) 0x04}, (String[]) null,
                true, null);

        // Создаём leaf, подписанный промежуточным
        TlsTestHelper.CertBundle leaf = TlsTestHelper.createCertSignedBy(
                params, intermediate.priv, intermediate.cert.getPublicKey(),
        ...
        byte[] intOcsp = TlsTestHelper.buildOcspResponse(
                intermediate.cert.getSerialNumber(), root.priv,
                root.cert.getPublicKey(), root.subjectDn);
        intermediate.cert.setOcspResponse(intOcsp);

        List<TlsCertificate> chain = List.of(leaf.cert, intermediate.cert, root.cert);
        GostX509TrustManager tm = new GostX509TrustManager(
                root.cert.getPublicKey(), OcspPolicy.STAPLING_OR_FETCH, null);
        tm.validateChainWithOcsp(chain, null, false);
    }
    ...
    void testIntermediateOcspMissing() throws Exception {
        ...
        assertFalse(intermediate.cert.hasOcspResponse(),
    ...
    private static TlsTestHelper.CertBundle createLeaf() throws Exception {
        return TlsTestHelper.createCertSignedBy(params, root.priv,
                root.cert.getPublicKey(), root.subjectDn,
                "240501120000Z", "290501120000Z",
                new String[]{"localhost"}, null, (String[]) null,
                false, null);
    }
}
