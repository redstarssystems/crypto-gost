...
class JdkHttpOcspFetcherTest {
    ...
    private static final byte[] SENTINEL =
            "OCSP_RESPONSE_OK".getBytes(java.nio.charset.StandardCharsets.UTF_8);
    ...
    private void startMock(byte[] responseBody, int statusCode) throws Exception {
        ...
        server.createContext(
                ...
                ex -> {
                    ...
                    assertTrue(body.length > 0, "Тело OCSP-запроса не должно быть пустым");
    ...
    void testFetchSuccess() throws Exception {
        ...
        assertArrayEquals(SENTINEL, result, "Fetcher должен вернуть контрольное тело");
    }
    ...
    void testFetchHttpsRejected() throws Exception {
        ...
        assertNull(
                fetcher.fetch(certDer, issuerDer, "https://ocsp.example.com/"),
                "HTTPS URI должны быть отклонены");
    }
