...
class GostSslContextBuilderTest {
    ...
    void testClientBuild() throws Exception {
        ...
        assertFalse(ctx.cipherSuites().isEmpty(), "cipher suites не должны быть пустыми");
    }
    ...
    void testClientWithoutTrustManagerFails() {
        SSLException ex = assertThrows(SSLException.class,
                () -> GostSslContextBuilder.forClient().build());
        assertTrue(ex.getMessage().contains("TrustManager"),
     ...
     */
    @Test
    @DisplayName("sessionTimeout(0) == unlimited")
    void testSessionTimeoutZero() throws Exception {
        ...
        assertEquals(0, timeout,
                "sessionTimeout(0) должен быть 0 (без ограничения по JSSE-контракту)");
    }
}
