...
class GostSslContextHandshakePskResumptionTest {
    ...
    static void setUp() throws Exception {
        ...
        serverCtx = GostSslContextBuilder.forServer(serverKm)
    ...
    @Test
    @DisplayName("PSK: full handshake → NST → reconnect → данные доходят")
    void testPskResumption() throws Exception {
        // ----- 1. Первое соединение — full handshake -----
        SslHandler clientSsl1 = clientCtx.newHandler(UnpooledByteBufAllocator.DEFAULT, "localhost", 0);
        SslHandler serverSsl1 = serverCtx.newHandler(UnpooledByteBufAllocator.DEFAULT, "localhost", 0);
        ...
        assertNotNull(decrypted, "данные после full handshake должны расшифроваться");
        assertEquals("hello", decrypted.toString(StandardCharsets.UTF_8));
        decrypted.release();

        // WHY: NST создан в finishHandshake() и ждёт в outgoingQueue.
        ...
        assertNotNull(nstRecord, "NST должен быть отправлен после full handshake");
        ...
        // WHY: isResumed() в GostSSLSession — JDK 9 default, всегда false.
        ...
        // WHY: RFC 8446 §8.1 — PSK-тикет одноразовый. Сервер сохраняет
        ...
        assertTrue(pskSizeBeforeClose > 0,
                "PSK должен быть сохранён после full handshake: " + pskSizeBeforeClose);

        // WHY: без drain close() сгенерирует close_notify outbound —
        ...
        // WHY: newHandler должен указывать host+port — иначе engine создастся
        // с peerHost=null и PSK lookup по null не найдёт NST из первого handshake.
        SslHandler clientSsl2 = clientCtx.newHandler(UnpooledByteBufAllocator.DEFAULT, "localhost", 0);
        SslHandler serverSsl2 = serverCtx.newHandler(UnpooledByteBufAllocator.DEFAULT, "localhost", 0);
        ...
        assertNotNull(decrypted, "данные после PSK-resumption должны расшифроваться");
