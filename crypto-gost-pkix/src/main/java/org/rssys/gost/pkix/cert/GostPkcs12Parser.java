...
public final class GostPkcs12Parser {
    ...
    public static final class PfxData {
        ...
        PfxData(
                int version,
                AuthenticatedSafeData authSafe,
                MacData macData,
        ...
        public AuthenticatedSafeData getAuthSafe() {
            return authSafe;
        }
        ...
        public byte[] getAuthSafeRawContent() {
            return authSafeRawContent;
        }
    }
    ...
    public static final class EncryptedPrivateKeyInfo {
        ...
        EncryptedPrivateKeyInfo(
                String encryptionAlgorithmOid, byte[] encryptionParams, byte[] encryptedData) {
    ...
    public static final class Pbes2Params {
        ...
        public String getEncryptionSchemeOid() {
            return encryptionSchemeOid;
        }
    ...
    public static final class SafeBagData {
        ...
        public byte[] getBagValue() {
            return bagValue;
        }

        public List<BagAttribute> getAttributes() {
            return attributes;
        }
    }
    ...
    public static final class BagAttribute {
        ...
        public BagAttribute(String attrId, byte[][] attrValues) {
            this.attrId = attrId;
    ...
    private static MacData parseMacData(byte[] data) {
        ...
        if (di.length < 2) {
            throw new IllegalArgumentException(
                    "MacData: DigestInfo must contain AlgorithmIdentifier and digest value");
        }
        byte[][] diAlgId = parseSequence(di[0], new int[] {0});
        if (diAlgId.length < 1) {
            throw new IllegalArgumentException(
                    "MacData: missing AlgorithmIdentifier in DigestInfo");
        }
    ...
    public static EncryptedPrivateKeyInfo parseEncryptedPrivateKeyInfo(byte[] data) {
        ...
        if (algId.length < 1) {
            throw new IllegalArgumentException(
                    "EncryptedPrivateKeyInfo: AlgorithmIdentifier missing OID");
        }
    ...
    // ========================================================================
    // DER traversal utilities — делегируют в DerCodec из core
    // ========================================================================
    ...
    public static byte[][] parseSequence(byte[] data, int[] pos) {
        return parseConstructed(data, pos, DerCodec.TAG_SEQUENCE);
    }

    private static byte[][] parseSet(byte[] data, int[] pos) {
        return parseConstructed(data, pos, DerCodec.TAG_SET);
    }

    private static byte[][] parseConstructed(byte[] data, int[] pos, int expectedTag) {
        byte[][] result;
        if (expectedTag == DerCodec.TAG_SEQUENCE) {
            result = DerCodec.parseSequenceContents(data, pos[0]);
        } else {
            result = DerCodec.parseSetContents(data, pos[0]);
        }
        int[] lenInfo = DerCodec.decodeLength(data, pos[0] + 1);
        pos[0] += 1 + lenInfo[1] + lenInfo[0];
        return result;
    }

    private static BigInteger parseInteger(byte[] data, int[] pos) {
        int offset = pos[0];
        checkTag(data, pos, DerCodec.TAG_INTEGER);
        int len = decodeLength(data, pos);
        BigInteger result = DerCodec.parseInteger(data, offset);
        pos[0] += len;
        return result;
    }
    ...
    public static String parseOid(byte[] data, int[] pos) {
        int offset = pos[0];
        checkTag(data, pos, DerCodec.TAG_OID);
        int len = decodeLength(data, pos);
        if (len == 0) {
            throw new IllegalArgumentException("Invalid OID: zero length at offset " + (pos[0]));
        }
        String result = DerCodec.parseOid(data, offset);
        pos[0] += len;
        return result;
    }
    ...
    private static byte[] parseOctetString(byte[] data, int[] pos) {
        int offset = pos[0];
        checkTag(data, pos, DerCodec.TAG_OCTET_STRING);
        int len = decodeLength(data, pos);
        byte[] result = DerCodec.parseOctetString(data, offset);
        pos[0] += len;
        return result;
    }
    ...
    private static byte[] unwrapContextSpecific(byte[] data) {
        int[] pos = new int[] {0};
        if ((data[pos[0]] & 0xFF) != DerCodec.TAG_CTX_BASE) {
    ...
    public static byte[] unwrapOctetString(byte[] data) {
        return DerCodec.parseOctetString(data, 0);
    }
    ...
    private static int decodeLength(byte[] data, int[] pos) {
        int[] result = DerCodec.decodeLength(data, pos[0]);
        pos[0] += result[1];
        return result[0];
    }

    private static void checkTag(byte[] data, int[] pos, int expectedTag) {
        DerCodec.checkTag(data, pos[0], expectedTag, "TAG 0x" + Integer.toHexString(expectedTag));
        pos[0]++;
    }
}
