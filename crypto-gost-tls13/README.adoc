...
Клиент передаёт свой сертификат и закрытый ключ через `withClientCertificate()`
(или `withClientCertificateChain(chain)` для полной цепочки) и
...
Пример загрузки из PKCS12-файла — рекомендуется для production (ключ и цепочка в одном файле):
...
**Ограничение:** GOST PBE не поддерживается — только PBES2 (PBKDF2-HMAC-SHA256 + AES-256-CBC).
...
- link:doc/architecture.md[crypto-gost-tls13/doc/architecture.md] — описание слоёв, ключевых классов, handshake flow, threading model, ограничений..
