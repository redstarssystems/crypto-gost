...
Типичный ГОСТ-сценарий: сервер требует сертификат с EKU УКЭП (`1.2.643.100.113.1`)
или УНЭП (`1.2.643.100.113.2`).
...
NOTE: Встроенные константы OID: `GostOids.EKU_UKEP` = `1.2.643.100.113.1`,
...
Созданный PFX читается `GostPkcs12Loader.load()` и проходит структурную валидацию OpenSSL.
