...
сертификат с EKU УКЭП (`1.2.643.100.113.1`) или УНЭП
(`1.2.643.100.113.2`).
...
Сервер — передать фильтры через `withOidFilters()`:
    ...
    // Фильтр: клиент должен предъявить сертификат с EKU УКЭП
    // filterOid = OID расширения EKU (2.5.29.37), filterValues = DER SEQUENCE { OID УКЭП }
    byte[] oidTlv = new byte[2 + TlsDerParser.UKEP_OID_BYTES.length];
    oidTlv[0] = 0x06; // TAG_OID
    oidTlv[1] = (byte) TlsDerParser.UKEP_OID_BYTES.length;
    System.arraycopy(TlsDerParser.UKEP_OID_BYTES, 0, oidTlv, 2, TlsDerParser.UKEP_OID_BYTES.length);
    ...
    OIDFilter ukepFilter = new OIDFilter(TlsDerParser.EKU_OID_BYTES, filterValues);

    TlsSession server = TlsSession.createServer(
            new TlsServerConfig(ciphersuite, chain, priv)
                    .withCaPublicKey(caKey)
                    .withOidFilters(List.of(ukepFilter)),
            transport);
    server.handshakeAsServer();
...
`withClientCertificateSelector()`:
    ...
    // Два сертификата: УКЭП (clientAuth + УКЭП EKU) и обычный (clientAuth)
    ClientCertificateSelector selector = (caDns, oidFilters) -> {
        for (var entry : List.of(
                Map.entry(ukepCert, ukepKey),
                Map.entry(simpleCert, simpleKey))) {
            TlsCertificate cert = entry.getKey();
            boolean ok = oidFilters.stream().allMatch(cert::matchesOidFilter);
...
Встроенные константы OID: `GostOids.EKU_UKEP` = `1.2.643.100.113.1`,
`GostOids.EKU_UNEP` = `1.2.643.100.113.2`. Байтовые константы:
`TlsDerParser.UKEP_OID_BYTES`, `TlsDerParser.UNEP_OID_BYTES`,
`TlsDerParser.EKU_OID_BYTES`.
...
цепочка в одном файле):
    ...
    List<TlsCertificate> chain = p12.getCertificateChain(); // leaf — первый элемент
...
`GostPkcs12Loader.load()` и проходит структурную валидацию OpenSSL.
