...
import org.rssys.gost.tls13.cert.GostPkcs12Parser.EncryptedPrivateKeyInfo;
import org.rssys.gost.tls13.cert.GostPkcs12Parser.Pbes2Params;
...
public final class GostPbes2 {
     ...
     */
    public static PrivateKeyParameters decryptKey(EncryptedPrivateKeyInfo epki,
                                                  byte[] password) {
        String encOid = epki.getEncryptionAlgorithmOid();
        if (!GostOids.PBES2.equals(encOid)) {
            throw new IllegalArgumentException(
                "Unsupported PBE algorithm: " + encOid
                + ". Expected PBES2 (" + GostOids.PBES2 + ")");
        }
    ...
    public static byte[] deriveKey(byte[] password, Pbes2Params params) {
        ...
        hmac.init(password);
        return Pbkdf2Streebog.generate(hmac, params.getSalt(),
            params.getIterationCount(), 32);
    }
     ...
     */
    public static byte[] encryptKey(PrivateKeyParameters privKey,
                                    byte[] password,
                                    String encSchemeOid,
                                    int iterations) {
        if (!GostOids.KUZ_CTR_ACPKM_OMAC.equals(encSchemeOid)
            ...
            throw new IllegalArgumentException(
                ...
                + ". Expected " + GostOids.KUZ_CTR_ACPKM_OMAC
                + " or " + GostOids.KUZ_CTR_ACPKM);
        }
        ...
        byte[] dk = Pbkdf2Streebog.generate(password, salt, iterations, 32);
        ...
        try {
            ...
            byte[] encryptedData;
            if (GostOids.KUZ_CTR_ACPKM_OMAC.equals(encSchemeOid)) {
                encryptedData = CtrAcpkmMode.encryptWithMac(
                    symKey, ukm, privateKeyInfoDer);
            } else {
                encryptedData = CtrAcpkmMode.decryptOnly(
                    symKey, ukm, privateKeyInfoDer);
            }
     ...
     */
    static byte[] buildPbes2Params(byte[] salt, int iterations,
                                    byte[] ukm, String prfOid, String encOid) {
        ...
        byte[] encAlgId = DerCodec.encodeSequence(
            DerCodec.encodeOid(encOid), encParams);
        byte[] kdfParams = DerCodec.encodeSequence(
            DerCodec.encodeOctetString(salt),
            DerCodec.encodeInteger(iterations),
            prfAlgId);
        byte[] kdfAlgId = DerCodec.encodeSequence(
            DerCodec.encodeOid(GostOids.PBKDF2), kdfParams);
    ...
    static byte[] buildEncryptedPrivateKeyInfo(byte[] pbes2Params, byte[] encryptedData) {
        byte[] algId = DerCodec.encodeSequence(
            DerCodec.encodeOid(GostOids.PBES2), pbes2Params);
        return DerCodec.encodeSequence(algId,
            DerCodec.encodeOctetString(encryptedData));
    }
