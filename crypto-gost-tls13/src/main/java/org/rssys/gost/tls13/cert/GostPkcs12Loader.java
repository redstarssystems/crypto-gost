...
import org.rssys.gost.tls13.cert.GostPkcs12Parser.ContentInfoData;
...
import org.rssys.gost.tls13.cert.GostPkcs12Parser.PfxData;
import org.rssys.gost.tls13.cert.GostPkcs12Parser.SafeBagData;
...
public final class GostPkcs12Loader {
    ...
    public static final class Result {
        ...
        public PrivateKeyParameters getPrivateKey() { return privateKey; }
        public List<TlsCertificate> getCertificateChain() { return certificateChain; }
    }
    ...
    private static Result loadGost(PfxData pfx, byte[] passwordBytes) {
        ...
        List<TlsCertificate> chain = filterCerts(rawCerts, keyLocalKeyId);
        chain = orderChain(chain);
        return new Result(privateKey, chain);
    }

    private static List<TlsCertificate> filterCerts(List<RawCert> rawCerts, byte[] keyLocalKeyId) {
        List<TlsCertificate> chain = new ArrayList<>();
        for (RawCert rc : rawCerts) {
            if (keyLocalKeyId != null && rc.localKeyId != null
                && !Arrays.equals(keyLocalKeyId, rc.localKeyId)) {
                continue;
            }
            chain.add(new TlsCertificate(rc.der));
        }
        if (chain.isEmpty()) {
            throw new IllegalArgumentException(
                "PKCS12: no certificates found after filtering");
        }
        return chain;
    }

    private static List<TlsCertificate> orderChain(List<TlsCertificate> chain) {
        int n = chain.size();
        if (n <= 1) return chain;
        ...
        List<TlsCertificate> result = new ArrayList<>(n);
    ...
    static PrivateKeyParameters adaptPrivateKey(PrivateKey jdkKey) {
