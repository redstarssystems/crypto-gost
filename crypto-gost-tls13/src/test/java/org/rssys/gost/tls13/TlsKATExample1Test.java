...
public class TlsKATExample1Test {
    ...
    private static final byte[] ECDHE =
            hex(
    ...
    private static final byte[] HM1 =
            hex("993BA722124AF3CBFD4771E7FAE32AC1D0E9278CF7843FCBC620E1A0085A87A1");

    // Transcript-Hash HM2 = Hash(ClientHello..Server Finished)
    private static final byte[] HM2 =
            hex("9EBC5FBE32D9F40D48F8EECEBB6231A533C2C0EF243277B96D6F7AD3BBFD1494");
    ...
    private static final byte[] EARLY_SECRET =
            hex("FBDEFBE527FEEA665AAB9277A2163B8343084FD191C46066260FAC6FD1436C72");

    // Derive-Secret(EarlySecret, "derived", "")
    private static final byte[] DERIVED_0 =
            hex("DBC3C826D877A3B7D2D2453DBFDC6CFBFB1151B3E84F0C8F26011D8D5BF3EDF7");

    // Шаг 2: HandshakeSecret = HKDF-Extract(Derived0, ECDHE)
    private static final byte[] HANDSHAKE_SECRET =
            hex("44245E2C4332D1F78B0F8D16F403EB69ED2A4053847CDC39FA8B3D2974F745E7");

    // Derive-Secret(HandshakeSecret, "derived", "")
    private static final byte[] DERIVED_1 =
            hex("EA3C54BBD14EF9D750776FABE395BE2ABDDBBBB71C13C2BD609E3515794AFA02");

    // Шаг 3: MainSecret = HKDF-Extract(Derived1, 0^32)
    private static final byte[] MAIN_SECRET =
            hex("31BB1D612CCD5332688A551A48CA250F24783D4AB0B4A76D3FE5067A2616A4A3");

    // Traffic secrets (Derive-Secret от HandshakeSecret / MainSecret)
    private static final byte[] SHTS =
            hex("70A5F2463DF60DBAA2368B67FD45AEFF7C1A0BA42D8ABD72415ECD1D94E9EF54");
    private static final byte[] CHTS =
            hex("B3F7113D3526554FE655E56FAB79B1A03DE33596E33088C7783719A9A4B0DCCD");
    private static final byte[] SATS =
            hex("87734F4B4CFD17B97B834D822D9D7379F6F5E03B80B52AEB2AFF510EDD83DBD2");
    private static final byte[] CATS =
            hex("8ACF746BEC31176CBD142C75806C270A0AEF6FC38E0D8FDCB5A88525363ADE81");

    // Finished keys (HKDF-Expand-Label от traffic secret, "finished", "")
    private static final byte[] SERVER_FINISHED_KEY =
            hex("53F1C0388F8A70C0BCA0DD21A030F2381C3437CD0E7EC93D0A965E25632DD79A");
    private static final byte[] CLIENT_FINISHED_KEY =
            hex("2F21548CF5277869AE490DE7BC15ACE639F657E3582A5A634B0A915695D54C42");

    // Handshake write keys/IVs (HKDF-Expand-Label от SHTS/CHTS)
    private static final byte[] SWK_HS =
            hex("E13764B54B9E1B47D43398D6D216DF24C289A396AB6C5B524BBB9C06F39FEF01");
    private static final byte[] SIV_HS = hex("6969FFAAA4525281EEBBEB4CBD0B640E");
    private static final byte[] CWK_HS =
            hex("581688D76EFE122BB55F62B38EF01BCC8C88DB83E9EA4D55D3898C53721FC384");
    private static final byte[] CIV_HS = hex("439A07453D0BEA0C1D1BEB738EB5B8DD");

    // Application write keys/IVs (HKDF-Expand-Label от SATS/CATS)
    private static final byte[] SWK_AP =
            hex("475E4C514CC6318C3A5F000F1265BD1AB5F0DE1AF357ED0079EC5FF0AFBD030C");
    private static final byte[] SIV_AP = hex("AFE91F7118354026317E1AB4D82217B8");
    private static final byte[] CWK_AP =
            hex("7BE64E2C12787B5B8C8756C43D92FAEF64F15A3A3C1081AD34BCA506F0322415");
    private static final byte[] CIV_AP = hex("310957EF71314433F576CC9B00AD9354");
    ...
    @Test
    @DisplayName("Вычисление EarlySecret = HKDF-Extract(0^32, 0^32)")
    ...
    @Test
    @DisplayName("Вычисление HandshakeSecret = HKDF-Extract(Derived0, ECDHE)")
    ...
    @Test
    @DisplayName("Вычисление MainSecret = HKDF-Extract(Derived1, 0^32)")
    void testMainSecret() {
        byte[] salt = DERIVED_1;
        byte[] ikm = new byte[32];
        byte[] actual = HkdfStreebog.extract(salt, ikm, 32);
    ...
    @Test
    @DisplayName("Вычисление Derived #0 = Derive-Secret(EarlySecret, \"derived\", Hash(\"\"))")
    void testDerived0() {
        byte[] actual = HkdfStreebog.deriveSecret(EARLY_SECRET, "derived", EMPTY_HASH, 32);
        assertArrayEquals(DERIVED_0, actual);
    }

    @Test
    @DisplayName("Вычисление Derived #1 = Derive-Secret(HandshakeSecret, \"derived\", Hash(\"\"))")
    void testDerived1() {
        byte[] actual = HkdfStreebog.deriveSecret(HANDSHAKE_SECRET, "derived", EMPTY_HASH, 32);
        assertArrayEquals(DERIVED_1, actual);
    }

    @Test
    @DisplayName(
            "Вычисление server_handshake_traffic_secret = Derive-Secret(HS, \"s hs traffic\", HM1)")
    void testServerHandshakeTrafficSecret() {
        byte[] actual = HkdfStreebog.deriveSecret(HANDSHAKE_SECRET, "s hs traffic", HM1, 32);
        assertArrayEquals(SHTS, actual);
    }

    @Test
    @DisplayName(
            "Вычисление client_handshake_traffic_secret = Derive-Secret(HS, \"c hs traffic\", HM1)")
    void testClientHandshakeTrafficSecret() {
        byte[] actual = HkdfStreebog.deriveSecret(HANDSHAKE_SECRET, "c hs traffic", HM1, 32);
        assertArrayEquals(CHTS, actual);
    }

    @Test
    @DisplayName(
            "Вычисление server_application_traffic_secret = Derive-Secret(MS, \"s ap traffic\", HM2)")
    void testServerAppTrafficSecret() {
        byte[] actual = HkdfStreebog.deriveSecret(MAIN_SECRET, "s ap traffic", HM2, 32);
        assertArrayEquals(SATS, actual);
    }

    @Test
    @DisplayName(
            "Вычисление client_application_traffic_secret = Derive-Secret(MS, \"c ap traffic\", HM2)")
    void testClientAppTrafficSecret() {
        byte[] actual = HkdfStreebog.deriveSecret(MAIN_SECRET, "c ap traffic", HM2, 32);
    ...
    @Test
    @DisplayName("Вычисление server_finished_key = HKDF-Expand-Label(SHTS, \"finished\", \"\", 32)")
    void testServerFinishedKey() {
        byte[] actual = HkdfStreebog.expandLabel(SHTS, "finished", new byte[0], 32, 32);
        assertArrayEquals(SERVER_FINISHED_KEY, actual);
    }

    @Test
    @DisplayName("Вычисление client_finished_key = HKDF-Expand-Label(CHTS, \"finished\", \"\", 32)")
    void testClientFinishedKey() {
        byte[] actual = HkdfStreebog.expandLabel(CHTS, "finished", new byte[0], 32, 32);
        assertArrayEquals(CLIENT_FINISHED_KEY, actual);
    }

    @Test
    @DisplayName("Вычисление server_write_key_hs = HKDF-Expand-Label(SHTS, \"key\", \"\", 32)")
    void testServerHandshakeWriteKey() {
        byte[] actual = HkdfStreebog.expandLabel(SHTS, "key", new byte[0], 32, 32);
        assertArrayEquals(SWK_HS, actual);
    }

    @Test
    @DisplayName("Вычисление server_write_iv_hs = HKDF-Expand-Label(SHTS, \"iv\", \"\", 16)")
    void testServerHandshakeWriteIv() {
        byte[] actual = HkdfStreebog.expandLabel(SHTS, "iv", new byte[0], 16, 32);
        assertArrayEquals(SIV_HS, actual);
    }

    @Test
    @DisplayName("Вычисление client_write_key_hs = HKDF-Expand-Label(CHTS, \"key\", \"\", 32)")
    void testClientHandshakeWriteKey() {
        byte[] actual = HkdfStreebog.expandLabel(CHTS, "key", new byte[0], 32, 32);
        assertArrayEquals(CWK_HS, actual);
    }

    @Test
    @DisplayName("Вычисление client_write_iv_hs = HKDF-Expand-Label(CHTS, \"iv\", \"\", 16)")
    void testClientHandshakeWriteIv() {
        byte[] actual = HkdfStreebog.expandLabel(CHTS, "iv", new byte[0], 16, 32);
        assertArrayEquals(CIV_HS, actual);
    }

    @Test
    @DisplayName("Вычисление server_write_key_ap = HKDF-Expand-Label(SATS, \"key\", \"\", 32)")
    void testServerAppWriteKey() {
        byte[] actual = HkdfStreebog.expandLabel(SATS, "key", new byte[0], 32, 32);
        assertArrayEquals(SWK_AP, actual);
    }

    @Test
    @DisplayName("Вычисление server_write_iv_ap = HKDF-Expand-Label(SATS, \"iv\", \"\", 16)")
    void testServerAppWriteIv() {
        byte[] actual = HkdfStreebog.expandLabel(SATS, "iv", new byte[0], 16, 32);
        assertArrayEquals(SIV_AP, actual);
    }

    @Test
    @DisplayName("Вычисление client_write_key_ap = HKDF-Expand-Label(CATS, \"key\", \"\", 32)")
    void testClientAppWriteKey() {
        byte[] actual = HkdfStreebog.expandLabel(CATS, "key", new byte[0], 32, 32);
        assertArrayEquals(CWK_AP, actual);
    }

    @Test
    @DisplayName("Вычисление client_write_iv_ap = HKDF-Expand-Label(CATS, \"iv\", \"\", 16)")
    void testClientAppWriteIv() {
        byte[] actual = HkdfStreebog.expandLabel(CATS, "iv", new byte[0], 16, 32);
    ...
    private static final byte[] TLSTREE_SRV_HS_0 =
            hex(
                    "56EE1813727249C9DCDF3513787EDB93DF62C61EE7B126C50F26C0AAAF AE00E1"
                            .replaceAll("\\s+", ""));
    // server application, seqNum 0 (root = SWK_AP)
    private static final byte[] TLSTREE_SRV_AP_0 =
            hex("C8FC93D7C586F2B0A3101BAA6A979E4E3886706551E81187E97880409C7E8EE9");
    // server application, seqNum 8 (root = SWK_AP, меняется на уровне 3)
    private static final byte[] TLSTREE_SRV_AP_8 =
            hex("D3CD87D5687407823978344C06B928A85898B739A31D3DE5FF2B788EF39196ED");
    // client handshake, seqNum 0 (root = CWK_HS)
    private static final byte[] TLSTREE_CLI_HS_0 =
            hex("E1C59B4169D896107F78456893A3751E1573543DAD8CB74069E6814A513BBB1C");
    // client application, seqNum 0 (root = CWK_AP)
    private static final byte[] TLSTREE_CLI_AP_0 =
            hex("D49A571549E748949FA24B8834232CA875D37A26C4BB5C62A261DAB372650526");
    // client application, seqNum 8 (root = CWK_AP)
    private static final byte[] TLSTREE_CLI_AP_8 =
            hex("B82D7825D15FAE18A7013228B31CB0C59752C6409C5F7899ECC6950F7463C090");

    @Test
    @DisplayName("TLSTREE: серверный handshake seqNum=0")
    ...
    @Test
    @DisplayName("TLSTREE: серверные данные приложения seqNum=0")
    ...
    @Test
    @DisplayName("TLSTREE: клиентский handshake seqNum=0")
    ...
    @Test
    @DisplayName("TLSTREE: клиентские данные приложения seqNum=0")
    ...
    @Test
    @DisplayName("TLSTREE: клиентские данные приложения seqNum=8")
    ...
    private static final byte[] HM_FINISHED =
            hex("03EC9B1D0B3741424572BAC9DF3AA52C03EFE9E958076943AFD85819BC602F46");

    // Server Finished verify_data = HMAC(server_finished_key, HM_FINISHED)
    private static final byte[] SERVER_VERIFY_DATA =
            hex("E0BAA33614E069697E4DFAB071B9725773F8FE1A326A662D0F52309B45B6E031");

    // Client Finished verify_data = HMAC(client_finished_key, TH2)
    private static final byte[] CLIENT_VERIFY_DATA =
            hex("085FC7FD79B6D111CD8D3FF6B23A065A7AF7A6387342A5F3576814CD004719D2");

    @Test
    @DisplayName("verify_data сервера = HMAC(server_finished_key, HMFinished)")
    void testServerVerifyData() {
        org.rssys.gost.mac.Hmac hmac =
                new org.rssys.gost.mac.Hmac(new org.rssys.gost.digest.Streebog256());
    ...
    private static final byte[] EE_CIPHERTEXT = hex("940E5D2C753AE5");
    ...
    void testEncryptedExtensionsRecord() {
        ...
        byte[] perRecordKey =
                TlsTree.deriveKey(SWK_HS, 0, C1, C2, C3, TlsConstants.KUZNYECHIK_KEY_SIZE);
        ...
        byte[] plaintext =
                new byte[] {0x08, 0x00, 0x00, 0x02, 0x00, 0x00, TlsConstants.CT_HANDSHAKE};
        byte[] aad =
                new byte[] {
                    ...
                    0x00,
                    (byte) (plaintext.length + TlsConstants.MGM_TAG_SIZE)
                };

        // 4. MGM-Encrypt
        org.rssys.gost.cipher.mode.Mgm mgm =
                new org.rssys.gost.cipher.mode.Mgm(new org.rssys.gost.cipher.Kuznyechik());
        mgm.init(
                true,
                new org.rssys.gost.cipher.ParametersWithIV(
    ...
    void testFullKeyScheduleChain() {
        ...
        assertArrayEquals(
                SHTS, ks.getServerHandshakeTrafficSecret(HM1), "Серверный HS трафик-секрет");
        assertArrayEquals(
                CHTS, ks.getClientHandshakeTrafficSecret(HM1), "Клиентский HS трафик-секрет");
        ...
        assertArrayEquals(SWK_HS, shKeys.getKey(), "ключ записи сервера (HS)");
        assertArrayEquals(SIV_HS, shKeys.getIv(), "IV записи сервера (HS)");
        ...
        assertArrayEquals(CWK_HS, chKeys.getKey(), "ключ записи клиента (HS)");
        assertArrayEquals(CIV_HS, chKeys.getIv(), "IV записи клиента (HS)");
        ...
        assertArrayEquals(SWK_AP, saKeys.getKey(), "ключ записи сервера (AP)");
        assertArrayEquals(SIV_AP, saKeys.getIv(), "IV записи сервера (AP)");
        ...
        assertArrayEquals(CWK_AP, caKeys.getKey(), "ключ записи клиента (AP)");
        assertArrayEquals(CIV_AP, caKeys.getIv(), "IV записи клиента (AP)");
        TlsUtils.wipeArray(caSecret);
        caKeys.destroy();

        // 7. Finished verify_data
        assertArrayEquals(
                SERVER_VERIFY_DATA, ks.computeVerifyData(SHTS, HM_FINISHED), "verify_data сервера");
        assertArrayEquals(
                CLIENT_VERIFY_DATA, ks.computeVerifyData(CHTS, HM2), "verify_data клиента");
    }
    ...
    @Test
    @DisplayName("ECDHE общий секрет: пример 2, GC256B")
    void testEcdheExample2() throws Exception {
        ...
        ECPoint serverQ = ECPoint.affine(new BigInteger(1, xBe), new BigInteger(1, yBe), params);
        PublicKeyParameters serverPub = new PublicKeyParameters(serverQ, params);

        byte[] shared = computeEcdheShared(clientPriv, serverPub, hlen);
        assertArrayEquals(
                ECDHE_GC256B,
                shared,
    ...
    private static byte[] computeEcdheShared(
            PrivateKeyParameters myPriv, PublicKeyParameters peerPub, int hashLen)
            throws Exception {
