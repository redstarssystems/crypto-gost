...
public class TlsKATLTest {
    ...
    // Regression only. No external KAT for Kuznyechik_L TLSTREE constants.
    private static final byte[] TLSTREE_L_SEQ0 =
            hex("F797256845F36CF075603445CD322BACC3834032BC425E4D3C8495236F7B6CAF");
    // Первичный прогон -> эталон. ZERO_KEY, seq=1 (0&1&C3=0 -> ключ как seq=0)
    private static final byte[] TLSTREE_L_SEQ1 =
            hex("F797256845F36CF075603445CD322BACC3834032BC425E4D3C8495236F7B6CAF");
    // Первичный прогон -> эталон. ZERO_KEY, seq=8191 (перед границей C3, &C3=0)
    private static final byte[] TLSTREE_L_C3_LO =
            hex("F797256845F36CF075603445CD322BACC3834032BC425E4D3C8495236F7B6CAF");
    // Первичный прогон -> эталон. ZERO_KEY, seq=8192 (после границы C3)
    private static final byte[] TLSTREE_L_C3_HI =
            hex("122CB2B5124F0BE01357810FBD63F1BBE7A1C6D73A3DDD8CD20A861D536B5E04");
    // Первичный прогон -> эталон. ZERO_KEY, seq=(1<<36)-1 (перед границей C2)
    private static final byte[] TLSTREE_L_C2_LO =
            hex("9709B581503B455B944C0E0A254567D3FECB35393C4CE2541E2C746696977DE5");
    // Первичный прогон -> эталон. ZERO_KEY, seq=1<<36 (после границы C2)
    private static final byte[] TLSTREE_L_C2_HI =
            hex("79B473D5176DDD026BF683875C3D2536998FB40E70630501D12395713F2BFD6F");

    @Test
    @DisplayName("TLSTREE-L: нулевой ключ, seq=0")
    ...
    @Test
    @DisplayName("TLSTREE-L: нулевой ключ, seq=1")
    ...
    void testL_EncryptedExtensionsRecord() {
        ...
        byte[] perRecordKey = TlsTree.deriveKey(SWK_HS, 0, C1_L, C2_L, C3_L, 32);
    ...
    void testL_EncryptedExtensionsRecord_Seq8192() {
        ...
        byte[] perRecordKey = TlsTree.deriveKey(SWK_HS, 8192L, C1_L, C2_L, C3_L, 32);
        ...
        assertArrayEquals(
                L_EE_CIPHERTEXT_SEQ8192,
                ciphertext,
                () ->
                        "C3 seq=8192 ct: ожидал "
                                + hexStr(L_EE_CIPHERTEXT_SEQ8192)
                                + ", получил "
                                + hexStr(ciphertext));
        assertArrayEquals(
                L_EE_TAG_SEQ8192,
                tag,
                () ->
                        "C3 seq=8192 tag: ожидал "
                                + hexStr(L_EE_TAG_SEQ8192)
                                + ", получил "
                                + hexStr(tag));
    }
}
