    ...
    public static void main(String[] args) throws Exception {
        ...
        for (int i = 0; i < args.length; i++) {
            if ("--port".equals(args[i])) {
                ...
                switch (curve) {
                    case "GC256B":
                        namedGroup = TlsConstants.GRP_GC256B;
                        break;
                    case "GC256A":
                        namedGroup = TlsConstants.GRP_GC256A;
                        break;
                    case "GC256C":
                        namedGroup = TlsConstants.GRP_GC256C;
                        break;
                    case "GC256D":
                        namedGroup = TlsConstants.GRP_GC256D;
                        break;
                    case "GC512A":
                        namedGroup = TlsConstants.GRP_GC512A;
                        break;
                    case "GC512B":
                        namedGroup = TlsConstants.GRP_GC512B;
                        break;
                    case "GC512C":
                        namedGroup = TlsConstants.GRP_GC512C;
                        break;
                    default:
        ...
        byte[] caKuExt =
                ExampleUtils.buildKeyUsageExt(new byte[] {0x06}); /* keyCertSign + cRLSign */
        ByteArrayOutputStream caExtBuf = new ByteArrayOutputStream();
        caExtBuf.write(caBcExt);
        caExtBuf.write(caKuExt);
        GostCertificate caCert =
                ExampleUtils.createCert(
                        ...
                        caDn,
                        caDn,
                        caExtBuf.toByteArray());
        byte[] caDer = caCert.getEncoded();
        try {
            java.nio.file.Files.write(
                    java.nio.file.Paths.get(
                            System.getProperty("gost.certs.dir", "/tmp"), "gost-ca.der"),
                    caDer);
        } catch (Exception e) {
        }
        try {
            ...
            java.nio.file.Files.writeString(
                    java.nio.file.Paths.get(
                            System.getProperty("gost.certs.dir", "/tmp"), "gost-ca.pem"),
                    "-----BEGIN CERTIFICATE-----\n" + b64 + "\n-----END CERTIFICATE-----\n");
        } catch (Exception e) {
        }
        ...
        if (java.nio.file.Files.exists(privPath) && java.nio.file.Files.exists(pubPath)) {
            ...
            try {
                java.nio.file.Files.write(
                        privPath, org.rssys.gost.jca.spec.GostDerCodec.encodePrivateKey(srvPriv));
            } catch (Exception e) {
            }
            try {
                java.nio.file.Files.write(
                        pubPath, org.rssys.gost.jca.spec.GostDerCodec.encodePublicKey(srvPub));
            } catch (Exception e) {
            }
        }
        ...
        byte[] kuExt =
                ExampleUtils.buildKeyUsageExt(new byte[] {(byte) 0x80}); /* digitalSignature */
        ...
        try {
            java.nio.file.Files.write(
                    java.nio.file.Paths.get(
                            System.getProperty("gost.certs.dir", "/tmp"), "gost-server.der"),
        ...
        String groupName =
                namedGroup == 0 ? "GC256B (default)" : "0x" + Integer.toHexString(namedGroup);
        ...
        if (mtls) {
            System.out.println("mTLS: required");
        }
            ...
            while (true) {
                try (Socket socket = ss.accept()) {
                    ...
                    TlsServerConfig config =
                            new TlsServerConfig(suite, Arrays.asList(serverCert, caCert), srvPriv);
                    if (namedGroup != 0) {
                        config.withSelectedNamedGroup(namedGroup);
                    }
                    if (clientCaFile != null) {
                        byte[] clientCaDer = Files.readAllBytes(Paths.get(clientCaFile));
                        PublicKeyParameters clientCaPub =
                                new GostCertificate(clientCaDer).getPublicKey();
                    ...
                    try (SocketTlsTransport transport = new SocketTlsTransport(socket);
                        ...
                        System.out.println(
                                "Negotiated: 0x"
                                        + Integer.toHexString(server.getCipherSuite().getId()));
                        System.out.println(
                                "Peer certs: "
                                        + (server.getPeerCertificates() != null
                                                ? server.getPeerCertificates().size()
                                                : 0));
