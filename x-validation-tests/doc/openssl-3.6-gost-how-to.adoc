...
* Два дополнительных патча исправляют верификацию сертификатов с ГОСТ-подписями (Streebog-256/512):
** `x509_set.c` — явные secbits для Streebog-256 (128) и Streebog-512 (256)
** `a_verify.c` — fallback на `EVP_MD_fetch` если `EVP_get_digestbynid` не находит дайджест
